...
import { ThemePromoBanner } from '@/components/ThemePromoBanner'
import {
  Star,
  User,
  Users,
...
const sideDict = {
  en: {
    ...
    descVisits: 'Hang out at the casa house.',
  ...
  es: {
    ...
    descVisits: 'Pasa el rato en la casa.',
      ...
      <div className="max-w-5xl mx-auto space-y-6 sm:space-y-8">
        <header className="flex items-center justify-between pb-4 border-b border-white/10 glass-panel px-4 sm:px-6 py-3 rounded-2xl sm:rounded-3xl shadow-lg">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <div className="hidden sm:flex items-center gap-2 sm:gap-3 pl-2 border-l border-white/10">
              <Link href="/suggest" className="p-2 rounded-xl glass-panel hover:bg-white/10 transition-colors text-zinc-300 hover:text-blue-400 touch-feedback" title="Suggestions"><Lightbulb className="w-4 h-4" /></Link>
              <Link href="/faq" className="p-2 rounded-xl glass-panel hover:bg-white/10 transition-colors text-zinc-300 hover:text-emerald-400 touch-feedback" title="FAQ"><HelpCircle className="w-4 h-4" /></Link>
              <Link href="/friends" className="p-2 rounded-xl glass-panel hover:bg-white/10 transition-colors text-zinc-300 hover:text-purple-400 touch-feedback" title="Friends"><Users className="w-4 h-4" /></Link>
              <Link href="/profile" className="flex items-center gap-2 hover:opacity-85 transition-opacity touch-feedback">
                <div className="w-9 h-9 rounded-full overflow-hidden border border-purple-500/40 bg-black flex items-center justify-center shadow-md">
                  {profile?.avatar_url ? <img src={profile.avatar_url} alt="Avatar" className="w-full h-full object-cover" /> : <User className="w-4 h-4 text-zinc-400" />}
                </div>
                <span className="text-xs font-bold text-zinc-200 hidden md:block">{profile?.username || 'Profile'}</span>
              </Link>
              {profile?.role === 'admin' && (
                <Link href="/admin" className="px-3 py-1.5 rounded-full border border-amber-500/30 bg-amber-500/15 hover:bg-amber-500/25 text-amber-400 text-[10px] uppercase font-black tracking-wider transition-colors shrink-0">Admin Panel</Link>
              )}
              <form action="/auth/signout" method="post">
                <button className="text-[10px] font-bold text-zinc-400 hover:text-red-400 transition-colors uppercase tracking-widest px-2">{t.logout}</button>
              </form>
        ...
        <footer className="mt-16 pt-8 pb-8 border-t border-white/10 text-center px-4">
          <div className="max-w-2xl mx-auto space-y-3">
            <p className="text-zinc-400 text-xs font-black uppercase tracking-widest">© 2026 Fandi Bank</p>
