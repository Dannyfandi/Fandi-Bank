    ...
    <div className="min-h-screen bg-transparent text-zinc-50 p-3 sm:p-5 md:p-8 font-sans pb-16">
      <div className="max-w-3xl mx-auto space-y-6 relative z-10">
        ...
        <main className="space-y-6">
          <div>
            <h1 className="text-2xl sm:text-3xl font-black tracking-tight bg-gradient-to-r from-emerald-400 to-cyan-400 bg-clip-text text-transparent flex items-center gap-2.5 text-shadow-sm">
              <HelpCircle className="w-6 h-6 text-emerald-400" /> {t.title}
            </h1>
            <p className="text-zinc-400 mt-1 text-xs sm:text-sm font-medium">{t.subtitle}</p>
          </div>
          ...
          <div className="grid grid-cols-1 gap-3.5">
            {faqs.map((faq, i) => (
              <div
                key={i}
                className="p-5 sm:p-6 glass-panel rounded-3xl border border-white/10 shadow-xl hover:border-emerald-500/30 transition-all group"
              >
                <h3 className="text-sm sm:text-base font-black text-white flex items-center gap-3 mb-2">
                ...
                <p className="text-xs sm:text-sm text-zinc-300 leading-relaxed font-medium pl-11 sm:pl-12">
                  {faq[lang].a}
                </p>
              </div>
            ))}
